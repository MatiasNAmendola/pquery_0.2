...
if( $task == 'add' ) 
{
	...
	$msg=$_GET['message'];
	if(!( strlen($msg)>1 && strlen($msg)<200))return;
...
<p>
<?=$pquery->form_remote_tag(array('id'=>'chatform','url'=>'index.php?task=add','success'=>'$("#chatarea").val(response+$("#chatarea").val());$("#chatmsg").val("");'));?>
...
<?=$pquery->periodically_call_remote(array('url'=>'index.php?task=refresh','success'=>'$("#chatarea").val(response+$("#chatarea").val());','frequency'=>2));?>;
<?=$pquery->ujs_cache;?>
});
</script>